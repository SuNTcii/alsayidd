...
local alsayidd = io.open("alsayidd", 'w')
alsayidd:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
lua5.3 alsayidd.lua
...
os.execute('chmod +x alsayidd;chmod +x alsayidd;./alsayidd')
end
...
alsayidd = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..alsayidd)
LuaTele = luatele.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=alsayidd,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(alsayidd.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(alsayidd) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(alsayidd.."chfalse")
if chh then
...
if msg.sender.user_id == tonumber(alsayidd) then
return false
end
if Redis:get(alsayidd.."chsource") then
chsource = Redis:get(alsayidd.."chsource")
else
chsource = "source_maas"
end
...
local Info_User = Redis:get(alsayidd.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(alsayidd..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'▹ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(alsayidd..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'▹ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "▹ الغاء الامر" then
Redis:del(alsayidd..msg.sender.user_id.."bottoken")
Redis:del(alsayidd..msg.sender.user_id.."botuser")
Redis:del(alsayidd..msg.sender.user_id.."make:bot")
...
Redis:set(alsayidd..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"▹ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(alsayidd..msg.sender.user_id.."stop:bot") then
Redis:del(alsayidd..msg.sender.user_id.."stop:bot")
Redis:del(alsayidd.."screen:on")
Redis:del(alsayidd.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(alsayidd..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"▹ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(alsayidd.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(alsayidd.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(alsayidd.."chfalse")
Redis:set(alsayidd.."chfalse",ch)
Redis:del(alsayidd.."ch:admin")
Redis:set(alsayidd.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'▹ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "▹ تفعيل الاشتراك الاجباري" then
Redis:set(alsayidd.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(alsayidd..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(alsayidd..msg.sender.user_id.."bottoken")
local botuser = Redis:get(alsayidd..msg.sender.user_id.."botuser")
...
Redis:sadd(alsayidd.."userbots",botuser)
Redis:del(alsayidd..msg.sender.user_id.."bottoken")
Redis:del(alsayidd..msg.sender.user_id.."botuser")
Redis:del(alsayidd..msg.sender.user_id.."make:bot")
...
if text and Redis:get(alsayidd..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(alsayidd.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n▹ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(alsayidd..msg.sender.user_id.."botuser",botuser)
Redis:set(alsayidd..msg.sender.user_id.."bottoken",text)
Redis:set(alsayidd..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(alsayidd..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n▹ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(alsayidd.."all:texting") then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(alsayidd.."all:texting")
send(msg.chat_id,msg.id, "\n▹ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(alsayidd.."3z:gp",text)
Redis:del(alsayidd.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(alsayidd.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(alsayidd.."3z:gp")
Redis:del(alsayidd.."all:texting")
send(msg.chat_id,msg.id,"▹ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(alsayidd.."all:texting:pv") then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(alsayidd.."all:texting:pv")
send(msg.chat_id,msg.id, "\n▹ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(alsayidd.."eza3a:pv",text)
Redis:del(alsayidd.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(alsayidd.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(alsayidd.."eza3a:pv")
Redis:del(alsayidd.."all:texting:pv")
send(msg.chat_id,msg.id,"▹ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "▹ اذاعه عام للمجموعات" then
Redis:set(alsayidd.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "▹ اذاعه عام للمشتركين" then
Redis:set(alsayidd.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "▹ فحص" then
Redis:del(alsayidd.."All:pv:st")
Redis:del(alsayidd.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(alsayidd.."All:gp:st",tonumber(#list))
Redis:incrby(alsayidd.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n▹ احصائيات جميع البوتات المصنوعه \n ▹ عدد المجموعات '..Redis:get(alsayidd.."All:gp:st")..' مجموعه \n▹ عدد المشتركين '..Redis:get(alsayidd.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
lt = Redis:smembers(bot_id.."Num:User:Pv") 
if #list < 2 then
Redis:sadd(alsayidd.."fake",botuser )
os.execute("sudo rm -fr "..botuser)
os.execute('screen -X -S '..botuser:gsub("@","")..' quit')
end
end
end
local list = Redis:smembers(alsayidd..'fake')
...
-------delete 
if text and Redis:get(alsayidd..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(alsayidd..msg.sender.user_id.."make:bot")
...
Redis:srem(alsayidd.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n▹ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "▹ حذف بوت" then
Redis:set(alsayidd..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(alsayidd..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(alsayidd..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(alsayidd..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(alsayidd..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n▹ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(alsayidd.."chsource",ch)
...
Redis:set(alsayidd..msg.sender.user_id.."setchannel",true)
send(msg.chat_id,msg.id,"▹ ارسل الان معرف القناه")
return false 
end
if text == "▹ تفعيل التواصل" then
Redis:del(alsayidd.."twsl")
send(msg.chat_id,msg.id,"▹ تم تفعيل التواصل")
return false 
end
if text == "▹ تعطيل التواصل" then
Redis:set(alsayidd.."twsl",true)
send(msg.chat_id,msg.id,"▹ تم تعطيل التواصل")
return false 
end
if text == "▹ الاحصائيات" then
local list = Redis:smembers(alsayidd.."total")
...
if tonumber(alsayidd) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'▹ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(alsayidd..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(alsayidd.."total")
local Get_Json = '{"BotId": '..alsayidd..','  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(alsayidd..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(alsayidd..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n▹ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(alsayidd.."total") 
...
if text == "▹ اذاعه" then
Redis:set(alsayidd..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"▹ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(alsayidd..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(alsayidd..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(alsayidd..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(alsayidd.."ch:admin")
send(msg.chat_id,msg.id,'\n▹ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(alsayidd.."twsl") then
if msg.sender.user_id ~= tonumber(alsayidd) then
if Redis:sismember(alsayidd..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(alsayidd.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(alsayidd.."free:bot") then
return send(msg.chat_id,msg.id,"▹ اهلا عزيزي تم تعطيل الوضع المجاني موعد تشغيل المصنع من الساعة 12 مساءً وحتى الساعة 12 ليلًا")
end
if text == "/start" then
if not Redis:sismember(alsayidd.."total",msg.sender.user_id) then
Redis:sadd(alsayidd.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
{text = '{ إنشاء بوت حمايه }',type = 'text'},{text = '{ مسح البوت }',type = 'text'},
},
{
{text = '{ محدث السورس }',type = 'text'},{text = '{ السورس }',type = 'text'},
},
{
...
send(msg.chat_id,msg.id,"▹ هلا بيـك في مصنع سوࢪس السيد يمكنك صنع بوت بكامل حقوقك مجانا \n▹ مبرمجين السورس  @cn22e - @F_FX1","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(alsayidd..msg.sender.user_id.."make:bot") then
if text == "▹ الغاء" then
Redis:del(alsayidd..msg.sender.user_id.."make:bot")
send(msg.chat_id,msg.id,"اهلا بيك عزيزي مره اخرى تم حذف بوتك بنجاح")
return false 
end
local url = https.request("http://api.telegram.org/bot"..text.."/getme")
local json = JSON.decode(url)
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(alsayidd..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(alsayidd.."userbots",botuser)
Redis:del(alsayidd..msg.sender.user_id.."make:bot")
...
if text == "{ إنشاء بوت حمايه }" then
if Redis:get(alsayidd..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"▹ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(alsayidd..msg.sender.user_id.."make:bot",true)
send(msg.chat_id,msg.id,"▹ ارسل توكن بوتك الان")
return false 
end
----end making user
if text == "{ مسح بوت }" then
if Redis:get(alsayidd..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(alsayidd..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(alsayidd.."userbots",botuser)
Redis:del(alsayidd..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n▹ تم حذف البوت بنجاح","md",true)  
else
...
----
if text == 'المطور سجاد' or text == '{ مبرمج السورس }' or text == '{ محدث السورس }' then  
local UserId_Info = LuaTele.searchPublicChat("cn22e")
...
{
{text = '1𝐌 𐀦 𝐒𝐉𝐀𝐃', url = "https://t.me/cn22e"}
},
}
...
{
{text = '◖| ٰٖ𝑎𝑙𝑠𝑎𝑦𝑖𝑑◖|🇮🇶|◗', url = "https://t.me/F_FX1"},
},
}
local msg_id = msg.id/2097152/0.5 
...
photo = "https://t.me/F_FX1/4"
local T =[[
welcome to the source alsayidd 
]]
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = 'ِّّسوࢪس السيد', url = 't.me/source_maas'}, 
},
}
...
if tonumber(data.message.sender.user_id) == tonumber(alsayidd) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == alsayidd then
return false
end
